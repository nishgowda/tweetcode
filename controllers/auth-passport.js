...
function(accessToken, refreshToken, profile, done) {
  ...
  conn.query(`select * from user where email=${conn.escape(profile._json['email'])}`, function(error, results){
    if (error) throw error;
    // if the user exists in database then grab their account
    if (results.length > 0){
       conn.query(`select * from user where uid=${conn.escape(results[0].uid)}`, function(err, result, f){
     ...
     // and grab their new account.
       ...
       conn.query(`insert into user(username, email, imageUrl) values(${conn.escape(profile.displayName)}, ${conn.escape(profile._json['email'])}, '${profile._json['avatar_url']}')`, function(err, data, field){
            if (err) throw err;
            conn.query(`select * from user where email=${conn.escape(profile._json['email'])}`, function(e, r, f){
               if (e) throw e;
               return done(null, r[0])
            });
        });
